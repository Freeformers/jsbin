...
module.exports = function (app) {
  ...
  var workstreamPushKey = app.get('freeformers workstreamPushKey');
  var workstreamPushUrl = app.get('freeformers workstreamPushUrl');
  var windowSize = 2000;

  console.log('Workstream settings', workstreamPushKey, workstreamPushUrl);

  if (workstreamPushUrl) {
    var binSubject = new Rx.Subject();
    binHandler.on('ff-bin-update', function (req, title) {
      binSubject.onNext({
        key: workstreamPushKey,
        type: 'update',
        binPermalink: req.bin.permalink,
        title: title,
        owner: req.bin.metadata.name,
        binCode: req.bin.metadata.url,
        commitHash: crypto.createHmac('sha256', req.bin.metadata.name).update(req.bin.html + req.bin.javascript + req.bin.css).digest('hex'),
        revision: req.bin.metadata.revision
      });
    });
    binHandler.on('ff-bin-create', function (req) {
      binSubject.onNext({
        key: workstreamPushKey,
        type: 'create',
        binPermalink: req.bin.permalink,
        title: req.bin.metadata.summary,
        owner: req.bin.metadata.name,
        binCode: req.bin.metadata.url,
        commitHash: crypto.createHmac('sha256', req.bin.metadata.name).update(req.bin.html + req.bin.javascript + req.bin.css).digest('hex'),
        revision: req.bin.metadata.revision
    ...
    var events = binSubject.windowWithTime(
      windowSize
    ).flatMap(
      function (window) {
    ...
    events.subscribe(
      function (stuff) {
        return fetch(
          ...
          console.log('Pushed to workstream', responseData);
        });
      }
    );
  }
